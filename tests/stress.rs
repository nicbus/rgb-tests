...
fn random_transfers() {
    ...
    //  - wallet list, with helper methods
    struct Wallets {
        wallets: Vec<RefCell<TestWallet>>,
    }
    impl Wallets {
        fn new() -> Self {
            Wallets { wallets: vec![] }
        ...
        fn push(&mut self, wallet: TestWallet) {
            self.wallets.push(RefCell::new(wallet));
        }

        fn get_mut(&mut self, idx: usize) -> &mut TestWallet {
            self.wallets[idx].get_mut()
        }
    }
    ...
    // - UTXO map: outpoint -> BTC amount (sats)
    type UTXOMap = HashMap<Outpoint, Sats>;
    ...
    fn choose_sender(
        ...
        for wi in wallet_idxs {
            ...
            // build contract map
            for c in contracts {
                let (cidx, ..) = assets.iter().find(|(_, (cid, _))| *cid == c.id).unwrap();
                let bal = balances.get(cidx).unwrap().get(wi).unwrap();
                if *bal > 0 {
        ...
        // return sender info
        (send_idx, contract_re_map, utxos, contract_allocations)
    }
    ...
    fn select_inputs_change_method(
        ...
        recipients.iter().for_each(|(_, cidx, _, amt)| {
            let cid = assets.get(cidx).unwrap().0;
            let entry = target_rgb_amounts.entry(cid).or_insert(0);
            *entry += amt;
        });
        // select RGB inputs
        ...
        remaining_utxos.retain(|(o, _)| !input_outpoints.contains_key(o));
        let remaining_utxos: Vec<(Outpoint, Sats)> =
            remaining_utxos.into_iter().map(|(o, s)| (*o, *s)).collect();
        let remaining_utxos: UTXOMap = HashMap::from_iter(remaining_utxos);
        ...
        let change_utxo = if change {
            ...
            if !remaining_utxos.is_empty() && rng.gen_bool(0.8) {
                ...
                Some(*remaining_utxos.iter().nth(change_utxo_idx).unwrap().0)
            } else {
                None
            }
        } else {
        ...
        for (r, cidx, tt, amt) in recipients {
            let destination = match tt {
                ...
                TransferType::Blinded => {
                    ...
                    let usable_utxos: HashMap<Outpoint, Sats> = wallets
                        .get_mut(*r)
                        .get_unspents()
                        .into_iter()
                    ...
                    let outpoint = if !usable_utxos.is_empty() && rng.gen_bool(0.8) {
                        ...
                        Some(usable_utxos.keys().nth(utxo_idx).unwrap())
                    } else {
                        None
                    };
                    AssetDestination::Blinded(
                        wallets.get_mut(*r).get_secret_seal(outpoint.cloned(), None),
                    )
                }
            };
            let contract_id = assets.get(cidx).unwrap().0;
    ...
    std::fs::create_dir_all(&stress_tests_dir).unwrap();
    let ts = OffsetDateTime::unix_timestamp(OffsetDateTime::now_utc()).to_string();
    let fname_base = format!(
        "random_transfers_seeded-{}-{}-{}-{}-{}",
        seed, loops, num_assets, num_wallets, ts
    );
    println!("\nreport files:");
    ...
    for _ in 0..num_wallets {
        ...
        let wlt = get_wallet(&descriptor_type);
        wallets.push(wlt);
    }

    // asset setup
    println!("\nissuing assets");
    let mut assets: Assets = BTreeMap::new();
    ...
    for i in 0..num_assets {
        ...
        assets.insert(i, (asset, schema.to_owned()));
        ...
        balances.insert(i, wallet_balances);
    ...
    for i in 1..=loops {
        ...
        let (send_idx, mut contract_re_map, utxos, contract_allocations) =
            choose_sender(&wallet_idxs, &mut wallets, &mut assets, &balances, &mut rng);
        ...
        for (r, cidx, _, amount) in &recipients {
            let asset_balances = balances.get_mut(cidx).unwrap();
        ...
        for (r, _, tt, _) in &recipients {
            if *r == send_idx {
                continue; // sender synced already
            }
            wallet_num += 1;
